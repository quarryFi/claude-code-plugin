...
5. For each profile, query the server status endpoint:
   ```bash
...
6. Read the local audit log at `~/.quarryfi/audit.log`. If it exists, show the last 20 entries:
   ```bash
...
8. Tell the user: "For detailed deduped R&D hours and qualification review, visit your QuarryFi dashboard: https://quarryfi.smashedstudiosllc.workers.dev/dashboard"

9. If the audit log doesn't exist or is empty, tell the user that no heartbeats have been sent yet — tracking starts on the next Claude Code session start.
